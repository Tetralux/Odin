...
Stream_Proc :: #type proc(stream_data: rawptr, mode: Stream_Mode, p: []byte, offset: i64, whence: Seek_From, loc: runtime.Source_Code_Location) -> (n: i64, err: Error)
...
destroy :: proc(s: Stream, loc := #caller_location) -> (err: Error) {
	_ = flush(s, loc)
	_ = close(s, loc)
	if s.procedure != nil {
		_, err = s.procedure(s.data, .Destroy, nil, 0, nil, loc)
	} else {
		err = .Empty
	}
	return
...
query :: proc(s: Stream, loc := #caller_location) -> (set: Stream_Mode_Set) {
	if s.procedure != nil {
		n, _ := s.procedure(s.data, .Query, nil, 0, nil, loc)
...
read :: proc(s: Reader, p: []byte, n_read: ^int = nil, loc := #caller_location) -> (n: int, err: Error) {
	if s.procedure != nil {
		n64: i64
		n64, err = s.procedure(s.data, .Read, p, 0, nil, loc)
		n = int(n64)
...
seek :: proc(s: Seeker, offset: i64, whence: Seek_From, loc := #caller_location) -> (n: i64, err: Error) {
	if s.procedure != nil {
		n, err = s.procedure(s.data, .Seek, nil, offset, whence, loc)
	} else {
		err = .Empty
	}
	return
...
close :: proc(s: Closer, loc := #caller_location) -> (err: Error) {
	if s.procedure != nil {
		_, err = s.procedure(s.data, .Close, nil, 0, nil, loc)
	}
	return
}

flush :: proc(s: Flusher, loc := #caller_location) -> (err: Error) {
	if s.procedure != nil {
		_, err = s.procedure(s.data, .Flush, nil, 0, nil, loc)
	}
	return
}

// size returns the size of the stream. If the stream does not support querying its size, 0 will be returned.
size :: proc(s: Stream, loc := #caller_location) -> (n: i64, err: Error) {
	if s.procedure != nil {
		n, err = s.procedure(s.data, .Size, nil, 0, nil, loc)
		if err == .Empty {
			n = 0
			curr := seek(s, 0, .Current, loc) or_return
			end  := seek(s, 0, .End, loc)     or_return
			seek(s, curr, .Start, loc)        or_return
			n = end
		}
	} else {
		err = .Empty
...
read_at :: proc(r: Reader_At, p: []byte, offset: i64, n_read: ^int = nil, loc := #caller_location) -> (n: int, err: Error) {
	if r.procedure != nil {
		n64: i64
		n64, err = r.procedure(r.data, .Read_At, p, offset, nil, loc)
		if err != .Empty {
			n = int(n64)
		} else {
			curr := seek(r, offset, .Current, loc) or_return
			n, err = read(r, p, loc = loc)
			_, err1 := seek(r, curr, .Start, loc)
			if err1 != nil && err == nil {
				err = err1
			}
		}
...
// seek offset.
write_at :: proc(w: Writer_At, p: []byte, offset: i64, n_written: ^int = nil, loc := #caller_location) -> (n: int, err: Error) {
	if w.procedure != nil {
		n64: i64
		n64, err = w.procedure(w.data, .Write_At, p, offset, nil, loc)
		if err != .Empty {
			n = int(n64)
		} else {
			curr := seek(w, offset, .Current, loc) or_return
			n, err = write(w, p, loc = loc)
			_, err1 := seek(w, curr, .Start, loc)
			if err1 != nil && err == nil {
				err = err1
			}
		}
...
read_byte :: proc(r: Reader, n_read: ^int = nil, loc := #caller_location) -> (b: byte, err: Error) {
	buf: [1]byte
	_, err = read(r, buf[:], n_read, loc)
	b = buf[0]
	return
}
...
read_rune :: proc(br: Reader, n_read: ^int = nil, loc := #caller_location) -> (ch: rune, size: int, err: Error) {
	defer if err == nil && n_read != nil {
		n_read^ += size
	}

	b: [utf8.UTF_MAX]byte
	_, err = read(br, b[:1], loc = loc)
	...
	sz := int(x&7)
	size, err = read(br, b[1:sz], loc = loc)
...
write_string16 :: proc(s: Writer, str: string16, n_written: ^int = nil, loc := #caller_location) -> (n: int, err: Error) {
	for i := 0; i < len(str); i += 1 {
		...
		w: int
		w, err = write_rune(s, r, n_written, loc)
		n += w
		if err != nil {
			return
		}
...
read_full :: proc(r: Reader, buf: []byte, loc := #caller_location) -> (n: int, err: Error) {
	return read_at_least(r, buf, len(buf), loc)
}
...
read_at_least :: proc(r: Reader, buf: []byte, min: int, loc := #caller_location) -> (n: int, err: Error) {
	if len(buf) < min {
		return 0, .Short_Buffer
	}
	for n < min && err == nil {
		nn: int
		nn, err = read(r, buf[n:], loc = loc)
		n += nn
	}
...
write_full :: proc(w: Writer, buf: []byte, loc := #caller_location) -> (n: int, err: Error) {
	return write_at_least(w, buf, len(buf), loc)
}
...
copy :: proc(dst: Writer, src: Reader, loc := #caller_location) -> (written: i64, err: Error) {
	return _copy_buffer(dst, src, nil, loc)
}
...
copy_buffer :: proc(dst: Writer, src: Reader, buf: []byte, loc := #caller_location) -> (written: i64, err: Error) {
	...
	return _copy_buffer(dst, src, buf, loc)
}
...
copy_n :: proc(dst: Writer, src: Reader, n: i64, loc := #caller_location) -> (written: i64, err: Error) {
	nsrc := limited_reader_init(&Limited_Reader{}, src, n)
	written, err = copy(dst, nsrc, loc)
	if written == n {
		return n, nil
	}
