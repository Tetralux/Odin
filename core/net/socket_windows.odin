...
join_multicast_group :: proc(skt: UDP_Socket, group: Multicast_Group) -> Network_Error {
	native_multicast_addr := endpoint_to_sockaddr(group.group_endpoint)
	native_interface_addr := endpoint_to_sockaddr({ address = group.interface_address })

	mreq: struct #raw_union {
		ipv4: win.ip_mreq,
		ipv6: win.ipv6_mreq,
	}
	res: c.int
	switch group.group_endpoint.address {
	case IP4_Address:
		mreq.ipv4 = win.ip_mreq {
			imr_multiaddr = (^win.sockaddr_in)(&native_multicast_addr).sin_addr,
			imr_interface = (^win.sockaddr_in)(&native_interface_addr).sin_addr,
		}
		res = win.setsockopt(Platform_Socket(skt), win.IPPROTO_IP, win.IP_ADD_MEMBERSHIP, &mreq, size_of(mreq.ipv4))
	case IP6_Address:
		mreq.ipv6 = win.ip6_mreq {
		...
		res = win.setsockopt(Platform_Socket(skt), win.IPPROTO_IP, win.IPV6_ADD_MEMBERSHIP, &mreq, size_of(mreq.ipv6))
	case:
...
leave_multicast_group :: proc(skt: UDP_Socket, group: Multicast_Group) -> Network_Error {
	native_multicast_addr := endpoint_to_sockaddr(group.group_endpoint)
	native_interface_addr := endpoint_to_sockaddr({ address = group.interface_address })
	...
	case IP4_Address:
		...
		res = win.setsockopt(Platform_Socket(skt), win.IPPROTO_IP, win.IP_DROP_MEMBERSHIP, &mreq, size_of(mreq.ipv4))
	case IP6_Address:
		mreq.ipv6 = win.ip6_mreq {
			ipv6mr_multiaddr = (^win.sockaddr_in6)(&native_multicast_addr).sin6_addr,
			ipv6mr_interface = (^win.sockaddr_in6)(&native_interface_addr).sin6_addr,
		}
		res = win.setsockopt(Platform_Socket(skt), win.IPPROTO_IP, win.IPV6_DROP_MEMBERSHIP, &mreq, size_of(mreq.ipv6))
	case:
